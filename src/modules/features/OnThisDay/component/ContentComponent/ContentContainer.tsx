		...
		containerTabs.map((eventType, index) => {
			return (
				<TabPanel key={`${eventType}-tab-panel`}>
					<Box {...contentBoxContainer} onScroll={(e) => handleScroll(e)}>
						<YearAccordian
							typeEvents={eventArticles[eventType as never]}
		...
		return (
			<Box {...boxContainer}>
				...
				>
					...
					<TabPanels>
						{isLoaded ? renderTabContainers() : renderContentSkeleton()}
					</TabPanels>
				</Tabs>
				<ScrollToTopButton
